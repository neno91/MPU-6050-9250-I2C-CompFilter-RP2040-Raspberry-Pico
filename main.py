...
class MPU:
    ...
    def setUp(self):
        ...
        self.i2c.writeto(self.address, bytearray([28, int(self.accHex)]))

        # Configure the gyro
        self.i2c.writeto(self.address, bytearray([27, int(self.gyroHex)]))
    ...
    def eightBit2sixteenBit(self, reg):
        ...
        h = self.i2c.readfrom_mem(self.address, reg, 1)
        l = self.i2c.readfrom_mem(self.address, reg+1, 1)
        self.i2c.stop()
        val = (ord(h) << 8) + ord(l)
